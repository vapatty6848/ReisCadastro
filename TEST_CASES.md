...
| :------ | :-------------------------- | :--------------------------------------------------------------- | :----------------------------------------------------------------------- |
| AUTH-01 | Login com Sucesso           | Inserir credenciais válidas (`admin@fanfarra.com` / `admin123`). | Redirecionamento para o Dashboard e armazenamento do token JWT.          |
...
| ID      | Caso de Teste              | Descrição                                                        | Resultado Esperado                                                           |
| :------ | :------------------------- | :--------------------------------------------------------------- | :--------------------------------------------------------------------------- |
| CRUD-01 | Cadastro Completo          | Preencher todos os campos (Aluno, Responsável, Escola) e salvar. | Aluno criado no banco, redirecionamento para listagem e mensagem de sucesso. |
| CRUD-02 | Cadastro com CPF Duplicado | Tentar cadastrar um aluno com um CPF já existente.               | Mensagem de erro "CPF já cadastrado" vinda do backend.                       |
...
| VAL-02 | Idade Mínima           | Tentar cadastrar aluno com data de nascimento futura. | Erro de validação de data.                                             |
...
| VAL-04 | Relacionamento Atômico | Simular erro no cadastro da Escola durante o envio.   | O Aluno e o Responsável não devem ser criados (Rollback da transação). |
...
| ID      | Caso de Teste             | Descrição                                          | Resultado Esperado                                            |
| :------ | :------------------------ | :------------------------------------------------- | :------------------------------------------------------------ |
| FILE-01 | Upload de Múltiplas Fotos | Selecionar 3 fotos válidas (JPG/PNG).              | Fotos enviadas para o servidor e exibidas no perfil do aluno. |
...
| ID      | Caso de Teste     | Descrição                                            | Resultado Esperado                                    |
| :------ | :---------------- | :--------------------------------------------------- | :---------------------------------------------------- |
| SRCH-01 | Pesquisa por Nome | Digitar parte do nome de um aluno no campo de busca. | A lista deve exibir apenas os alunos correspondentes. |
| SRCH-02 | Pesquisa por CPF  | Digitar o CPF exato de um aluno.                     | A lista deve filtrar o aluno específico.              |
| SRCH-03 | Filtro Vazio      | Pesquisar por um termo que não existe.               | Exibição de mensagem "Nenhum integrante encontrado".  |
| SRCH-04 | Limpar Filtros    | Realizar uma busca e depois limpar o campo.          | A lista completa de alunos deve retornar.             |
