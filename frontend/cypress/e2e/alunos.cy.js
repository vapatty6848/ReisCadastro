describe('Gestão de Integrantes (CRUD)', () => {
  const alunoNome = 'Cypress Test Aluno ' + Date.now();
  ...
  beforeEach(() => {
    ...
    cy.request('POST', 'http://localhost:3001/api/auth/login', {
      ...
      cy.window().then((win) => {
        win.localStorage.setItem('@Fanfarra:token', token);
        win.localStorage.setItem('@Fanfarra:user', JSON.stringify(user));
      });
    });

    cy.visit('/dashboard/alunos');
  });

  it('deve cadastrar um novo integrante com sucesso', () => {
    cy.intercept('POST', '**/api/alunos').as('createAluno')
    ...
    // Dados do Aluno
    cy.get('input[name="nome"]').type(alunoNome)
    cy.get('input[name="cpf"]').type(alunoCpf)
    ...
    // Escola
    cy.get('input[name="escola.nome"]').type('Escola Cypress')
    cy.get('input[name="escola.telefone"]').type('1133334444')
    ...
    cy.wait('@createAluno').its('response.statusCode').should('eq', 201)

    // Verifica se voltou para a listagem
    cy.url().should('include', '/dashboard/alunos')
    cy.contains(alunoNome).should('be.visible')
  })

  it('deve pesquisar e editar um integrante', () => {
    cy.intercept('PATCH', '**/api/alunos/*').as('updateAluno')

    cy.visit('/dashboard/alunos')
    cy.wait(2000)

    // Pesquisa pelo nome criado no teste anterior
    cy.get('input[placeholder="Pesquisar por nome..."]').type(alunoNome)
  ...
  it('deve excluir um integrante', () => {
    cy.intercept('DELETE', '**/api/alunos/*').as('deleteAluno')
    ...
    cy.wait('@deleteAluno').its('response.statusCode').should('eq', 204)

    cy.contains(alunoNome).should('not.exist')
  })
