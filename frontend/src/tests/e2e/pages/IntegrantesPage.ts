...
export class IntegrantesPage {
  ...
  // Locators - Corporação
  readonly corpNomeInput: Locator;
  ...
  constructor(page: Page) {
    ...
    // Corporação
    this.corpNomeInput = page.locator('input[name="corporacao.nome"]');
    ...
    this.patrimonioSearchInput = page.locator('input[placeholder="Filtrar por patrimônio..."]');
  }
  ...
  async salvarAlteracoes() {
    const responsePromise = this.page.waitForResponse(res => res.status() === 200);
  ...
  async excluir() {
    ...
    const responsePromise = this.page.waitForResponse(
      res => res.url().includes('/api/integrantes') && res.request().method() === 'DELETE'
    );
    await this.page.click('button[title="Excluir"]');
    await responsePromise;
  }

  async selecionarStatusFiltro(status: 'DEVOLVIDO' | 'NAO_DEVOLVIDO' | 'TODOS') {
  ...
  async preencherIdentificacao(data: any) {
    ...
    if (data.dataNascimento) await this.dataNascimentoInput.fill(data.dataNascimento);
  ...
  async preencherCorporacao(data: any) {
    if (data.nome) await this.corpNomeInput.fill(data.nome);
    if (data.telefone) await this.corpTelefoneInput.fill(data.telefone);
    if (data.dataMatricula) await this.dataMatriculaInput.fill(data.dataMatricula);
  }

  async preencherAtuacao(data: any) {
    if (data.tipo) await this.tipoSelect.selectOption(data.tipo);
    if (data.subtipo) await this.subtipoSelect.selectOption(data.subtipo);
