...
test.describe("Exemplos do Plano de Testes", () => {
  ...
  test("AUTH-02: deve exibir erro ao inserir senha incorreta", async ({
    page,
  }) => {
  ...
  test("SRCH-03: deve exibir mensagem quando nenhum resultado for encontrado", async ({
    page,
  }) => {
    await loginAndSetStorage(page);
